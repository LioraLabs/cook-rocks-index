...
description = {
   ...
   detailed = [[
cook_ai 0.2.0-2 — validation moved from register-time to execute-time so
`cook_ai.provider({...})` can be called at top level (or omitted entirely)
even when model / api_key / base_url come from cook.env via config blocks
or shell env. Reads from cook.env.{COOK_AI_MODEL, COOK_AI_PROVIDER,
COOK_AI_BASE_URL, ANTHROPIC_API_KEY} inside cook_ai.prompt() at execute
time. Pairs with the cook engine's Lua-side cook.env auto-cache (Cook
Standard §17.1.2, CS-0090) so editing any of those keys invalidates
every cook_ai-bearing unit automatically.
