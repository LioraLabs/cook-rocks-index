...
description = {
   ...
   detailed = [[
      ...
      dhewm3-cook (COOK-2) — the Cookfile carried one `local cfg = ...`
      ...
      (COOK-2) — the resulting link command had `\n-lcurl -lz` and bash
      ...
      `build/bin/base.so` consumed via `dlopen` — see COOK-2). Authors
