...
source = {
   ...
   tag = "cook_cc-0.20.0-1",
}
description = {
   summary  = "Cook C-family (C + C++) native build module",
   detailed = [[
      0.20.0: probe declarations drop `inputs.env`, which CS-0244 removed from
      cook and which cook now rejects outright at register phase. Every project
      on 0.19.x fails at `cook menu` against a cook carrying CS-0244, because
      `cc:linker-search-dirs` declares LIBRARY_PATH before anything else runs.

      The declarations are deleted rather than replaced, and no determinant is
      lost. `env` only ever fed the probe fingerprint, which CS-0244 also
      deleted; it never fed a probe's value. Each variable still reaches the
      cache key by the sharper route, through the value it moves:
      LIBRARY_PATH resolves into the output of `cc -print-search-dirs`;
      PKG_CONFIG_PATH, CMAKE_PREFIX_PATH and PATH act on what `cc:find:<name>`
      resolves, and that resolution is the value; the compiler behind
      `cc:check:*` is already hashed by the toolchain probe it requires. On
      `cc:cmake-driver`, CMAKE_PREFIX_PATH never moved the value at all. A
      variable that moves without changing what it resolves to no longer
      invalidates anything downstream.

      0.19.1 is the Language v2 terminology pass. Every place the module
      named `ingredients` now names `gather`: the iteration declaration that
      survived the v2 cut. `ingredients` is not a spelling the engine still
      accepts — CS-0229 deleted it, and a Cookfile using it is now met with a
      migration diagnostic — so a module comment describing a recipe in terms
      of it sent readers at a keyword that no longer parses.

      Prose and specs only. No API, no generated Cookfile text and no runtime
      behaviour differs from 0.19.0; build.modules is unchanged, regenerated
      from disk to prove it.

      0.19.0 adds cook_cc's project-management verbs: commands a person
      types, alongside the makers a Cookfile calls.

      cc.new scaffolds a working C++ project in place around an existing
      Cookfile — sources, headers, tests, .clang-format, the managed
      .gitignore section, and build/run/test recipes — so `cook build &&
      cook run` works with no editing. It refuses rather than clobbering: a
      pre-existing src/main.cpp, an already-declared build/run/test recipe,
      or a toolchain call that would silently downgrade the project's C++
      standard.

      cc.add scaffolds and registers a lib or bin (`cook cc.add lib math`),
      kind first, validated against a table that shared and headers extend
      later. The generated lib recipe carries includes = { "include" } so
      the scaffold compiles on the first build.

      cc.link wires link edges (`cook cc.link game math sound`), creating
      the links field when the target has none. It is idempotent, and a
      swapped argument order self-heals with the corrected command rather
      than erroring.

      cc.need declares an external dependency and probes for it now,
      splicing both the needs entry and the top-level uses() declaration
      that makes it resolvable, then reporting what the finder chain found.

      cc.fmt runs clang-format over src/, include/ and tests/. It refuses
      unless the project declares a style, rather than silently imposing
      LLVM defaults on a tree that never adopted clang-format.

      All five are module-registered chores (Standard §22.12) whose bodies
      edit the Cookfile through the §22.13 byte-preserving primitives. They
      need a cook providing CS-0218 (a chore body runs only when asked
      for), CS-0220 (modules install wires the use line) and CS-0221 (a
      Cookfile edit may create the field it was asked to add to).

      The contract the five share is written down in src/verbs/README.md.
      build.modules gains the whole cook_cc.verbs.* subtree, regenerated
      from disk — which is what 0.18.0-1 failed to do.

      0.18.0-2 is a repackaging of 0.18.0-1, which omitted
      cook_cc.units.object_path and cook_cc.units.sources (added by
      CS-0167 after the 0.17.0 rockspec was cut) from build.modules and
      could not load at all. No source change. build.modules is now
      verified against the tree.

      0.18.0 is the engine-HEAD parity pass (cook @ CS-0192..CS-0198): an
      end-to-end sweep against the post-unification engine found zero
      engine breaks and three latent module bugs, all fixed here.
      checks.has_header was always false for dotted headers since 0.6.1 —
      the key-sanitised name (stdint_h) leaked into the generated
      #include; the raw name now flows to the probe body, the sanitised
      form stays key-only, and the raw name folds into the fingerprint so
      sys/stat.h and sys_stat.h cannot share a key. checks.has_function
      was always false since first release — its extern function-pointer
      reference was never defined, so the link-kind check failed
      unconditionally; it is a definition now. config_header's render
      unit embedded the absolute renderer path from package.searchpath,
      re-keying the unit on every directory rename (against CS-0196
      portable identity) and silently requiring a host lua on PATH; it is
      a lua_code unit now that require()s the renderer by module name on
      the worker VM, and check values arrive typed via cook.probes.get,
      which also fixes string-valued checks (endian()) in vars. Cache
      impact: check probes re-key once (name in fingerprint) and
      config_header units re-key once (command -> lua_code); everything
      else, including compile commands, is unchanged. Locked by six new
      specs (306 total); validated by cold/warm/incremental builds of the
      DuckDB tree (1711 units) on engine main.
...
build = {
   ...
   modules = {
     ...
     ["cook_cc.discovery.finders"]                    = "cook_cc/src/discovery/finders/init.lua",
     ...
     ["cook_cc.toolchain"]                            = "cook_cc/src/toolchain/init.lua",
     ["cook_cc.toolchain.version"]                    = "cook_cc/src/toolchain/version.lua",
     ["cook_cc.units.cc"]                             = "cook_cc/src/units/cc.lua",
     ["cook_cc.units.object_path"]                    = "cook_cc/src/units/object_path.lua",
     ["cook_cc.units.sources"]                        = "cook_cc/src/units/sources.lua",
     ["cook_cc.units.targets"]                        = "cook_cc/src/units/targets.lua",
     ["cook_cc.units.transitive"]                     = "cook_cc/src/units/transitive.lua",
     ["cook_cc.verbs"]                                = "cook_cc/src/verbs/init.lua",
     ["cook_cc.verbs._project"]                       = "cook_cc/src/verbs/_project.lua",
     ["cook_cc.verbs._report"]                        = "cook_cc/src/verbs/_report.lua",
     ["cook_cc.verbs.add"]                            = "cook_cc/src/verbs/add.lua",
     ["cook_cc.verbs.fmt"]                            = "cook_cc/src/verbs/fmt.lua",
     ["cook_cc.verbs.link"]                           = "cook_cc/src/verbs/link.lua",
     ["cook_cc.verbs.need"]                           = "cook_cc/src/verbs/need.lua",
     ["cook_cc.verbs.new"]                            = "cook_cc/src/verbs/new.lua",
   },
}
