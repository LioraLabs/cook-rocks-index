...
source = {
   ...
   tag = "cook_pnpm-0.10.0-1",
}
description = {
   summary  = "Cook pnpm blessed module with namespaced workspace tasks",
   detailed = [[
      ...
      pnpm-lock.yaml hash), mints one pnpm:<dir>:<task> recipe per
      (package, task) in topological order, auto-mints conventional
      checks, and mints the install stage.

      0.10.0 changes minted recipe names from npm package names to a stable
      module-owned namespace. Root tasks are `pnpm:<task>` and workspace
      members are `pnpm:<directory-basename>:<task>`, so scoped npm names no
      longer leak `@scope/` into Cook's command surface and module tasks cannot
      collide with handwritten `build` or `lint` recipes. Duplicate member
      directory basenames are rejected at registration instead of silently
      overwriting recipes. Commands and per-package override lookup continue
      to use the full npm package name.

      0.9.0: the `pnpm:find:<name>` probe drops its `inputs.env` declaration,
      which CS-0244 removed from cook and which cook now rejects at register
      phase. The break is latent rather than immediate here: no Cookfile in the
      wild reaches `pnpm.find`, so a workspace keeps registering, and the first
      project to call the finder would have been the one to discover it.

      Deleting rather than replacing loses nothing. `env` only fed the probe
      fingerprint, which CS-0244 also deleted; PATH still reaches the cache key
      through `tools`, which hashes the resolved binary, and through the value
      itself, which carries the path and version the produce body resolved.

      The module had no finder spec at all. It has one now, guarding the input
      declaration, because the absence of a downstream consumer is exactly what
      makes a reintroduction here invisible until it reaches a user.
