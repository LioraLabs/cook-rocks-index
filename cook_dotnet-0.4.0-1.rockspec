package = "cook_dotnet"
version = "0.4.0-1"
source = { url = "git+https://github.com/lioralabs/cook-modules.git", tag = "cook_dotnet-0.4.0-1" }
description = {
   summary = "Fresh, cacheable .NET builds for Cook",
   detailed = [[
      0.4.0 adds workspace.root: consumers can bound project and configuration
      discovery to an explicit subtree. Projects discovered outside that root
      fail with an instruction to expand it, preserving cache soundness while
      avoiding unrelated repository walks (COOK-560).
